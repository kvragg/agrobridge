-- ============================================================
-- AgroBridge — Fix ambiguous column reference em confirmar_pagamento_v2 (v3)
-- ============================================================
-- A migration 20260421070000_rpc_confirmar_pagamento_v3.sql REGRIDE o fix
-- aplicado em 20260421040000_fix_ambiguous_vars.sql:
--
--   1. SELECT bare `user_id` voltou no snapshot do processo (linha 76 do
--      arquivo v3) — colide com OUT param `user_id` do RETURNS TABLE.
--   2. Nova gate de Mentoria adicionada na v3 contém:
--         SELECT COUNT(*) ... FROM public.compras
--         WHERE tier = 'mentoria' AND status = 'paid' ...
--      `tier` é OUT param do RETURNS TABLE — também colide.
--
-- Sintoma (capturado em UAT 2026-04-21 via /admin/compras "Reprocessar
-- compra manual"): primeira chamada da RPC retorna 42702
-- "column reference \"user_id\" is ambiguous" e endpoint devolve 500.
--
-- Correção cirúrgica: alias de tabela em todos os SELECT que tocam
-- colunas com nome igual a OUT param. Mesma assinatura (8 args) e
-- mesmo retorno — consumidor TS continua igual.
-- ============================================================
...
BEGIN
  ...
  -- Snapshot do processo. Alias `p.` evita ambiguidade com OUT params
  -- (user_id no RETURNS TABLE).
  ...
  -- Gate de limite mensal para Mentoria. Alias `c.` evita ambiguidade
  -- com OUT param `tier` do RETURNS TABLE.
  IF p_tier = 'mentoria' THEN
    SELECT COUNT(*)::INTEGER
      INTO v_vagas_usadas
    FROM public.compras c
    WHERE c.tier = 'mentoria'
      AND c.status = 'paid'
      AND c.paid_at >= date_trunc('month', NOW())
      AND c.paid_at <  date_trunc('month', NOW()) + interval '1 month'
